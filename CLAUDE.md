...
- PLA-11 complete: Drag-and-drop seating canvas live — `SeatingWorkspace.tsx` wraps `DndContext`, guests are draggable, tables are droppable, couples move atomically, optimistic UI, seat count + name chips on tables, over-capacity turns table red, writes to `seat_assignments` in Supabase
