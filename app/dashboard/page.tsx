...
export default async function Dashboard() {
  ...
  // Fetch guest counts and seated counts per project
  const projectIds = (projects ?? []).map(p => p.id)
  const [{ data: guestRows }, { data: seatedRows }] = projectIds.length
    ? await Promise.all([
        supabase.from('guests').select('project_id').in('project_id', projectIds),
        supabase.from('seat_assignments').select('guest_id, guests!inner(project_id)').not('table_id', 'is', null),
      ])
    : [{ data: [] }, { data: [] }]

  const guestsByProject: Record<string, number> = {}
  for (const row of guestRows ?? []) {
    guestsByProject[row.project_id] = (guestsByProject[row.project_id] ?? 0) + 1
  }

  const seatedByProject: Record<string, number> = {}
  for (const row of (seatedRows ?? []) as any[]) {
    const pid = row.guests?.project_id
    if (pid) seatedByProject[pid] = (seatedByProject[pid] ?? 0) + 1
  }
              ...
              {(projects as Project[]).map((project) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs tabular-nums" style={{ color: '#8a7f74' }}>
                      {project.type === 'single'
                        ? `Single event · ${seatedByProject[project.id] ?? 0} / ${guestsByProject[project.id] ?? 0} seated`
                        : 'Multiple events'}
                    </span>
                  </div>
                </Link>
